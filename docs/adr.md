...
## 2026-09-19 - Define the panel welcome text once, and add a Japanese README

- Task: Issue #249.
- Decision:
  - Move the welcome block text into `src/chatWelcomeText.ts`, a module with no `vscode` import, and render it from there in both places it appears: the initial HTML built by `ChatViewProvider.getHtmlForWebview()` and the DOM rebuilt by `ChatRenderer.clear()`. Keep the text from the extension host version, which describes the current behavior, and keep the "combine source commands" tip that only the webview version carried.
  - Add `README_ja.md`, a full Japanese translation of `README.md`, and cross-link the two. `SECURITY.md` and `LICENSE` stay English only.
